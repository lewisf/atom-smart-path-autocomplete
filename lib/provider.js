...
export default class AutocompletePathProvider {
  ...
  findSuggestionsForPrefix(prefix, resolve) {
    ...
    var suggestions = paths.map(path => {
      // TODO: Remove static/ from being hardcoded and change it to a configurable option with a default
      return {
        displayText: path.replace('static/', ''),
        text: handleFileExtension(path).replace('static/', ''),
