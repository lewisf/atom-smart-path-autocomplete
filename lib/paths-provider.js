...
const config = atom.config.get('atom-smart-paths-autocomplete');
...
export default class PathsProvider {
  ...
  getPrefixForCursor(editor, bufferPosition) {
    ...
    var start = this.getBeginningOfCurrentWordBufferPosition(editor, bufferPosition);
  ...
  findSuggestionsForPrefix(prefix, resolve) {
    ...
    var suggestions = _.map(paths, path => {
      return {
        displayText: path.replace(config.staticRoot, ''),
        text: handleFileExtension(path).replace(config.staticRoot, ''),
